...
void Stats::displayStats()
{
    ...
    for (size_t i = 0; i < _movesMade.size(); i++)
    {
        if (i != 0)
            std::cout << ", ";
        std::cout << "{ " << _movesMade[i].x << " " << _movesMade[i].y << " " << _movesMade[i].z << " }";
    }
    std::cout << std::endl;

    std::cout << "MovesGenerated: ";
...
void Stats::updateRawGameDataSheet(int turn, int userTurn)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < _statisticsData.size(); i++)
        {
            if (i != 0)
                strStream2 << ",";
            strStream2 << "(" << i << "," << _statisticsData[i] << ")";
        }
        movesGenerated = strStream2.str();
